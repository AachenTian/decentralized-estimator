...
def _masked_rmse(
    ...
    """RMSE over selected transitions and state dimensions."""
    ...
    if errors.ndim != 2:
        raise ValueError(
            f"errors must have shape (B, D), got {errors.shape}."
        )

    if mask.shape != (errors.shape[0],):
        raise ValueError(
            f"mask must have shape {(errors.shape[0],)}, "
            f"got {mask.shape}."
        )
    ...
    return float(
        jnp.sqrt(
            jnp.mean(
                jnp.square(errors[mask])
            )
        )
    )


def collision_conditioned_metrics(
    predicted_next_states: jnp.ndarray,
    target_next_states: jnp.ndarray,
    collision_flags: jnp.ndarray,
) -> dict[str, float]:
    """Split one-step prediction errors by collision involvement."""
    ...
    noncollision = jnp.logical_not(collision)
    errors = predicted - target

    return {
        "validation_collision_local_rate": float(
            jnp.mean(collision)
        ),
        "validation_collision_count": int(
            jnp.sum(collision)
        ),
        "validation_noncollision_count": int(
            jnp.sum(noncollision)
        ),

        "validation_collision_state_rmse":
            _masked_rmse(
                errors,
                collision,
            ),
        "validation_noncollision_state_rmse":
            _masked_rmse(
                errors,
                noncollision,
            ),

        "validation_collision_position_rmse":
            _masked_rmse(
                errors[:, :2],
                collision,
            ),
        "validation_noncollision_position_rmse":
            _masked_rmse(
                errors[:, :2],
                noncollision,
            ),

        "validation_collision_velocity_rmse":
            _masked_rmse(
                errors[:, 2:4],
                collision,
            ),
        "validation_noncollision_velocity_rmse":
            _masked_rmse(
                errors[:, 2:4],
                noncollision,
            ),

...
class JaxMARLSimpleSpreadAdapter:
    ...
    def compute_collision_metrics(
        ...
        Returns:
            ...
            agent_collision_mask: Per-agent collision involvement with shape
                (..., num_agents).
        """
        ...
        off_diagonal = jnp.logical_not(
            jnp.eye(n, dtype=jnp.bool_)
        )

        # Symmetric pairwise collision matrix. The diagonal is excluded so an
        # agent is never considered to collide with itself.
        collision_matrix = (
            distances < self.config.collision_distance
        ) & off_diagonal

        # Per-agent label used by collision-conditioned dynamics evaluation.
        # Shape: (..., num_agents). True means this agent is involved in at
        # least one collision at the current state.
        agent_collision_mask = jnp.any(
            collision_matrix,
            axis=-1,
        )
        ...
        colliding_pairs = collision_matrix & upper_triangle
        ...
        return {
            ...
            "agent_collision_mask": agent_collision_mask,
        }

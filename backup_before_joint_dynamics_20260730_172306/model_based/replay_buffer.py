...
class PooledLocalDynamicsBuffer:
    ...
    Each row contains only one agent's local state and action. The collision
    flag is metadata for conditional validation; it is never a model input.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.local_states = np.empty(
            (capacity, local_state_dim), dtype=np.float32
        )
    ...
    def add_batch(
        ...
        *,
        ...
        if local_states.ndim != 2 or local_states.shape[1] != self.local_state_dim:
            raise ValueError(
        ...
        if collision_flags is None:
            collision_flags = np.zeros((batch_size,), dtype=np.bool_)
        else:
            collision_flags = np.asarray(
                collision_flags, dtype=np.bool_
            ).reshape(-1)
        ...
        if (
            ...
            raise ValueError("Dynamics batch sizes do not match.")
    ...
    def batch_from_indices(self, indices) -> LocalDynamicsBatch:
        ...
        return LocalDynamicsBatch(
            ...
            collision_flags=jnp.asarray(self.collision_flags[indices]),
        )

...
def plot_xy_three_way(
    ...
    for agent_id in range(truth.shape[1]):
        role = "ego" if agent_id == ego_agent_id else "remote"

        plt.plot(
            truth[:, agent_id, 0],
            truth[:, agent_id, 1],
            label=f"agent {agent_id} true ({role})",
        )
        plt.plot(
    ...
    plt.ylabel("y")
    plt.title(
        "True vs model-only vs covariance-triggered estimator"
    )
    plt.axis("equal")
    plt.grid(True)
    plt.legend()
...
def plot_remote_position_errors(
    ...
    plt.title("Remote position error and communication events")
    plt.grid(True)
    plt.legend()
    save_figure(output, dpi)
...
def plot_position_uncertainty_radii(
    ...
    plt.title(
        "Belief position-covariance trigger signal"
    )
    plt.grid(True)
    plt.legend()
    save_figure(output, dpi)
...
def main() -> None:
    ...
    plot_xy_three_way(
        ...
        ego_agent_id=args.ego_agent_id,
        output=args.output_directory / "xy_three_way.png",
        dpi=args.dpi,
    )

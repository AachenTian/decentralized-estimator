...
## Phase-two dynamics smoke test

Phase two implements three independent five-member dynamics ensembles. It does
not yet connect synthetic transitions to SAC.

Run:

```bash
bash scripts/test.sh
rm -rf outputs/phase2_dynamics_smoke
bash scripts/dynamics_smoke_test.sh
```

Expected terminal accounting remains:
...
metrics.jsonl
phase2_dynamics_smoke_summary.json
wandb/offline-run-*/
```

Core W&B metrics appear under:

```text
owner/0/dynamics/...
owner/1/dynamics/...
owner/2/dynamics/...
system/dynamics_updates_total
```

The actor calibration rollout is public and separate. Each dynamics normalizer
is fitted only from its owner's nonterminal private replay and then frozen.

## Phase 3

Run `bash scripts/model_rollout_smoke_test.sh`. The smoke test uses 32 initial states and horizon 2, so each owner can generate at most 64 synthetic transitions. Early termination can reduce this count.

## Phase 4

Phase four adds three independent owner-local SAC learners. Run:

```bash
bash scripts/test.sh
rm -rf outputs/phase4_independent_sac_smoke
bash scripts/sac_smoke_test.sh
```

Expected high-level result:

```text
owner 0: critic_updates=10, actor_updates=10
owner 1: critic_updates=10, actor_updates=10
owner 2: critic_updates=10, actor_updates=10
snapshot_bank_unchanged=true
```

See `PHASE4_STATUS.md`.

## Formal training

Phase five adds formal training, deterministic evaluation, best/latest
checkpoints, numbered actor checkpoints, and exact resume support.

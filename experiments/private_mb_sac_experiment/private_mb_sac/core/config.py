...
def validate_config(config: Mapping[str, Any]) -> None:
    required = (
        ...
        "sac.policy_delay",
        "sac.minimum_replay_size",
        "tracking.project",
        "tracking.mode",
        "environment_rng.forced_seed",
        "animation.rounds",
        "animation.horizon",
        "animation.fps",
    )
    ...
    positive_keys = (
        ...
        "sac.policy_delay",
        "sac.minimum_replay_size",
        "sac.fixed_batch_size",
        "actor.maximum_gradient_norm",
        "critic.maximum_gradient_norm",
        "temperature.maximum_gradient_norm",
        "animation.horizon",
        "animation.fps",
        "animation.dpi",
        "animation.trail_length",
    )
    invalid = {
    ...
    if int(_require(config, "environment_rng.forced_seed")) != 40:
        raise ValueError(
            "This diagnostic experiment requires environment seed 40."
        )
    required_animation_rounds = {0, 10, 30, 60, 90, 120, 150, 180, 190, 200}
    configured_animation_rounds = {
        int(value) for value in _require(config, "animation.rounds")
    }
    if configured_animation_rounds != required_animation_rounds:
        raise ValueError(
            "animation.rounds must equal "
            f"{sorted(required_animation_rounds)}."
        )

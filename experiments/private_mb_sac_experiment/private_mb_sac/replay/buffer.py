...
class PrivateJointReplayBuffer:
    ...
    def state_dict(self) -> dict:
        """Return a compact serializable snapshot of this private replay."""
        size = int(self._size)
        names = (
            "observations",
            "actions",
        ...
        return {
            ...
            "size": size,
            "arrays": {
                name: np.asarray(getattr(self, name)[:size]).copy()
                for name in names
            },
        }

    def load_state_dict(self, payload: dict) -> None:
        """Restore a compact replay snapshot into this existing object."""
        expected = {
            "capacity": self.capacity,
            "num_agents": self.num_agents,
            "observation_dim": self.observation_dim,
        ...
        actual = {key: int(payload[key]) for key in expected}
        if actual != expected:
            raise ValueError(
                f"Replay metadata mismatch: expected={expected}, actual={actual}."
            )

        size = int(payload["size"])
        position = int(payload["position"])
        if not 0 <= size <= self.capacity:
            raise ValueError(f"Invalid replay size: {size}.")
        if not 0 <= position < self.capacity:
            raise ValueError(f"Invalid replay position: {position}.")

        for name, values in payload["arrays"].items():
            array = np.asarray(values)
            if array.shape[0] != size:
                raise ValueError(
                    f"Replay field {name} has {array.shape[0]} items; expected {size}."
                )
            getattr(self, name)[:size] = array

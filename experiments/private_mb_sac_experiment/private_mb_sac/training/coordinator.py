...
class OwnerRuntime:
    ...
    terminal_transitions_total: int = 0
...
def collect_private_round(
    *,
    ...
    snapshot_bank: Any | None = None,
) -> tuple[list[dict[str, float]], dict[str, float]]:
    """Collect one round for all owners and append to private replays.

    When supplied, `snapshot_bank` is reused by collection, model rollout, and
    every local SAC update in the same synchronization round.
    """
    snapshots = snapshot_bank
    ...
    for runtime in owner_runtimes:
        ...
        terminal_transitions_round = int(episode_dones.sum())
        runtime.terminal_transitions_total += terminal_transitions_round
        ...
        owner_metrics.append(
            {
                ...
                "real/episodes_round": float(terminal_transitions_round),
                "real/terminal_transitions_round": float(
                    terminal_transitions_round
                ),
                "real/terminal_transitions_total": float(
    ...
    system_metrics = {
        ...
        "terminal_transitions_round": float(
            sum(
                metrics["real/terminal_transitions_round"]
                for metrics in owner_metrics
            )
        ),
        "terminal_transitions_total": float(
            sum(
                runtime.terminal_transitions_total
                for runtime in owner_runtimes
            )

...
def _new_private_buffer(args, spec) -> JointReplayBuffer:
    return JointReplayBuffer(
        capacity=args.effective_buffer_capacity,
...
def _save_checkpoint(
    ...
    *,
    ...
    payload = {
        ...
        "private_replay_persistent": True,
        "private_replay_policy": "recent_circular_window",
        "private_replay_capacity": args.effective_buffer_capacity,
        "private_replay_retention_rounds_approx": (
            args.effective_replay_retention_rounds
        ),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Per-agent persistent circular replay capacity. Use 0 to derive "
            "capacity as replay-retention-rounds times one private rollout "
            "round."
        ),
    )
    parser.add_argument(
        "--replay-retention-rounds",
        type=int,
        default=8,
        help=(
            "Approximate number of recent synchronization rounds retained "
            "when buffer-capacity is 0."
        ),
    )
    ...
    positive = {
        ...
        "replay_retention_rounds": args.replay_retention_rounds,
    ...
    requested_buffer_capacity = int(args.buffer_capacity)
    effective_buffer_capacity = (
        requested_buffer_capacity
        if requested_buffer_capacity > 0
        else private_transitions_per_round * args.replay_retention_rounds
    )
    if effective_buffer_capacity < private_transitions_per_round:
        raise ValueError(
            "Persistent buffer capacity must hold at least one full private "
            "rollout round: "
            f"need at least {private_transitions_per_round}, got "
            f"{effective_buffer_capacity}."
        )
    if private_transitions_per_round < max(
        args.batch_size, args.fixed_batch_size
    ):
        raise ValueError(
            "The first private rollout round must contain at least batch-size "
            "and fixed-batch-size transitions."
        )
    args.effective_buffer_capacity = int(effective_buffer_capacity)
    args.effective_replay_retention_rounds = float(
        effective_buffer_capacity / private_transitions_per_round
    )
    ...
    critic_sample_reuse_ratio = (
        args.gradient_steps * args.batch_size
        / float(private_transitions_per_round)
    )
    actor_sample_reuse_ratio = (
        critic_sample_reuse_ratio / float(args.policy_delay)
    )
    ...
    print("Private replay: persistent independent circular buffer per agent")
    print(f"Private replay capacity per agent: {effective_buffer_capacity}")
    print(
        "Approximate recent rounds retained: "
        f"{args.effective_replay_retention_rounds:.2f}"
    )
    print(
    ...
    print(f"Critic gradient-step/data ratio per owner: {critic_utd_ratio:.6f}")
    print(f"Post-warmup actor gradient-step/data ratio: {actor_utd_ratio:.6f}")
    print(
        "Critic minibatch-sample reuse per new transition: "
        f"{critic_sample_reuse_ratio:.3f}"
    )
    print(
        "Post-warmup actor minibatch-sample reuse per new transition: "
        f"{actor_sample_reuse_ratio:.3f}"
    )
    ...
    # Each learner owns one replay buffer for the whole run. New transitions
    # are appended every synchronization round. Once full, the circular
    # buffer overwrites the oldest transitions, retaining a recent window.
        ...
        json.dump(
            {
                ...
                "private_replay_per_agent": True,
                "private_replay_persistent": True,
                "private_replay_lifetime": "recent_circular_window",
                "private_replay_capacity_per_agent": (
                    effective_buffer_capacity
                ),
                "private_replay_retention_rounds_approx": (
                    args.effective_replay_retention_rounds
                ...
                "critic_sample_reuse_ratio_per_owner": (
                    critic_sample_reuse_ratio
                ),
                "actor_sample_reuse_ratio_per_owner": (
                    actor_sample_reuse_ratio
                ),
            },
            handle,
    ...
    for round_index in range(1, args.total_sync_rounds + 1):
        ...
        # Strict private data ownership is preserved, but each private buffer
        # persists across rounds. Appending this round's transitions gradually
        # overwrites only the oldest data after the circular capacity is full.
        round_rollouts = []
        ...
        metrics: dict[str, float | int | bool] = {
            ...
            "private_replay_refreshed_this_round": False,
            "private_replay_persistent": True,
            "private_replay_capacity_per_agent": effective_buffer_capacity,
            "private_replay_retention_rounds_approx": (
                args.effective_replay_retention_rounds
            ),
            "private_replay_new_transitions_per_owner": (
                private_transitions_per_round
            ),
            "critic_utd_ratio_per_owner": critic_utd_ratio,
            "actor_utd_ratio_per_owner": actor_utd_ratio,
        ...
        for agent_id in range(spec.num_agents):
            ...
            replay_size = len(private_buffers[agent_id])
            metrics[f"agent{agent_id}_private_replay_size"] = replay_size
            metrics[f"agent{agent_id}_private_replay_fill_fraction"] = (
                replay_size / float(effective_buffer_capacity)
            )
            metrics[f"agent{agent_id}_private_replay_rounds_equivalent"] = (
                replay_size / float(private_transitions_per_round)
            )
        ...
        if do_evaluation:
            print(
                ...
                f"replay={metrics['agent0_private_replay_size']}/"
                f"{effective_buffer_capacity} "

...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train parameter-sharing IPPO on an environment adapter. "
            "This first version uses oracle remote states to validate the "
            "policy optimizer independently of the estimator."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/ippo_simple_spread"),
    )
    ...
    spec = adapter.spec
    print("EnvSpec:", spec)
    print("Training mode: parameter-sharing IPPO with local critic")
    ...
    dummy_critic_obs = jnp.zeros_like(dummy_actor_obs)

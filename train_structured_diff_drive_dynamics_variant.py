...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Fraction of the original training trajectories to use. "
            "Using the same --train-subset-seed makes fractions nested."
        ),
    )
    parser.add_argument(
        "--train-subset-seed",
        type=int,
        default=2026,
        help="Seed used only for the nested training-trajectory subset.",
    )
    parser.add_argument(
        "--holdout-dimension",
        choices=("none", "px", "py", "phi", "v", "omega"),
        default="none",
        help=(
            "Optionally remove training transitions from one side of a "
            "state-space quantile. For this structured model, v or omega "
            "is more meaningful than px/py because position is reconstructed "
            "analytically and is not a learned input."
        ),
    )
    parser.add_argument(
        "--holdout-quantile",
        type=float,
        default=0.8,
        help="Quantile used to define the held-out state region.",
    )
    parser.add_argument(
        "--holdout-side",
        choices=("upper", "lower"),
        default="upper",
        help="Remove values above (upper) or below (lower) the quantile.",
    )
    parser.add_argument(
        "--metrics-json",
        type=Path,
        default=None,
        help="Optional path for final one-step and rollout metrics.",
    )
...
def select_nested_train_indices(
    ...
    if not 0.0 < fraction <= 1.0:
        raise ValueError("train-data-fraction must be in (0, 1].")
    original = np.asarray(original_indices, dtype=np.int64)
    rng = np.random.default_rng(seed)
    permutation = rng.permutation(original)
    count = max(1, int(np.ceil(fraction * len(original))))
    return np.sort(permutation[:count])


def compute_holdout_threshold(
    local_states,
    ...
    if not 0.0 < quantile < 1.0:
        raise ValueError("holdout-quantile must be in (0, 1).")
    values = np.asarray(local_states)[
        np.asarray(trajectory_indices), :-1, :, dimension_index
    ].reshape(-1)
...
def agent_transitions(
    ...
    if holdout_dimension_index is not None:
        if holdout_threshold is None:
            raise ValueError("holdout_threshold is required.")
        values = current[:, holdout_dimension_index]
        if holdout_side == "upper":
            keep = values <= holdout_threshold
        else:
            keep = values >= holdout_threshold
        ...
        if int(current.shape[0]) == 0:
            raise RuntimeError(
                f"Holdout removed every transition for agent {agent_id}."
            )
...
def main():
    ...
    original_train_indices = np.asarray(splits["train"], dtype=np.int64)
    selected_train_indices = select_nested_train_indices(
    ...
    holdout_dimension_index = None
    holdout_threshold = None
    if args.holdout_dimension != "none":
        holdout_dimension_index = STATE_DIMENSIONS[args.holdout_dimension]
        holdout_threshold = compute_holdout_threshold(
            local_states,
            selected_train_indices,
            holdout_dimension_index,
            args.holdout_quantile,
        )
    ...
    print("original train trajectories:", len(original_train_indices))
    print("selected train trajectories:", len(selected_train_indices))
    print("train data fraction:", args.train_data_fraction)
    print("train subset seed:", args.train_subset_seed)
    if holdout_dimension_index is not None:
        print(
            "held-out state region:",
            f"{args.holdout_dimension} {args.holdout_side} ",
            f"quantile={args.holdout_quantile:g}, ",
            f"threshold={holdout_threshold:.6f}",
        )
    ...
    train_batches = [
        agent_transitions(
            ...
            selected_train_indices,
            agent_id,
            holdout_dimension_index=holdout_dimension_index,
            holdout_threshold=holdout_threshold,
            holdout_side=args.holdout_side,
        )
        for agent_id in range(num_agents)
    ]
    test_batches = [
        ...
        checkpoint_metadata.update(
            {
                ...
                "train_trajectory_indices": (
                    selected_train_indices.tolist()
                ),
                "original_train_trajectory_indices": (
                    original_train_indices.tolist()
                ),
                "train_data_fraction": args.train_data_fraction,
                "train_subset_seed": args.train_subset_seed,
                "holdout_dimension": args.holdout_dimension,
                "holdout_dimension_index": holdout_dimension_index,
                "holdout_quantile": (
                    args.holdout_quantile
                    if holdout_dimension_index is not None
    ...
    if args.metrics_json is not None:
        ...
        summary = {
            ...
            "train_data_fraction": args.train_data_fraction,
            "train_subset_seed": args.train_subset_seed,
            "selected_train_trajectory_indices": (
                selected_train_indices.tolist()
            ),
            "holdout_dimension": args.holdout_dimension,
            "holdout_quantile": (

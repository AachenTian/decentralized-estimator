...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "results/figures/structured_covariance_trigger_estimator/"
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "results/animations/structured_covariance_trigger_estimator"
        ),
    )
    parser.add_argument(
        "--animation-name",
        default="structured_covariance_trigger_estimator.mp4",
    )
...
def load_trace(path: Path) -> dict[str, Any]:
    ...
    required = {
        ...
        "communications",
        "ego_agent_id",
        "position_trigger_radius",
        "position_trigger_scale",
    }
    ...
    if trace["communications"].shape != (horizon, num_agents):
        raise ValueError("communications must have shape (H, N).")
...
def plot_all_agents_three_way(
    ...
    for agent_id in range(truth.shape[1]):
        ...
        axis.plot(
            truth[:, agent_id, 0], truth[:, agent_id, 1],
            color=color, linewidth=2.6, linestyle="-",
        )
        axis.plot(
            model_only[:, agent_id, 0], model_only[:, agent_id, 1],
            color=color, linewidth=1.9, linestyle="--", alpha=0.78,
        )
        axis.plot(
            posterior[:, agent_id, 0], posterior[:, agent_id, 1],
            color=color, linewidth=2.1, linestyle="-.", alpha=0.95,
        )
    ...
    agent_handles = [
        Line2D(
            ...
            label=f"agent {i} ({'ego' if i == ego_agent_id else 'remote'})",
        )
        for i in range(truth.shape[1])
    ]
    method_handles = [
        Line2D([0], [0], color="#222222", linewidth=2.5, linestyle="-", label="true"),
        Line2D([0], [0], color="#222222", linewidth=2.0, linestyle="--", label="model-only"),
        Line2D([0], [0], color="#222222", linewidth=2.0, linestyle="-.", label="estimator posterior"),
        Line2D([0], [0], color=COMMUNICATION_COLOR, marker="D", markerfacecolor="none", linestyle="None", label="communication"),
        Line2D([0], [0], color=START_COLOR, marker="o", linestyle="None", label="start"),
        Line2D([0], [0], color=END_COLOR, marker="X", linestyle="None", label="end"),
    ]

    legend_agents = axis.legend(handles=agent_handles, loc="upper left", title="Agent color")
    axis.add_artist(legend_agents)
    axis.legend(handles=method_handles, loc="lower right", title="Meaning")

    axis.set_xlabel("x position")
    axis.set_ylabel("y position")
    axis.set_title("All agents: true, model-only, and corrected estimator")
...
def plot_all_agents_true_estimator(
    ...
    for agent_id in range(truth.shape[1]):
        ...
        axis.plot(
            truth[:, agent_id, 0], truth[:, agent_id, 1],
            color=color, linewidth=2.7, linestyle="-",
        )
        axis.plot(
            posterior[:, agent_id, 0], posterior[:, agent_id, 1],
            color=color, linewidth=2.1, linestyle="--", alpha=0.92,
        )
        add_start_end(axis, truth[:, agent_id, :2])
    ...
    method_handles = [
        Line2D([0], [0], color="#222222", linewidth=2.5, linestyle="-", label="true"),
        Line2D([0], [0], color="#222222", linewidth=2.1, linestyle="--", label="estimator posterior"),
        Line2D([0], [0], color=COMMUNICATION_COLOR, marker="D", markerfacecolor="none", linestyle="None", label="communication"),
        Line2D([0], [0], color=START_COLOR, marker="o", linestyle="None", label="start"),
        Line2D([0], [0], color=END_COLOR, marker="X", linestyle="None", label="end"),
    ]
    ...
    axis.set_xlabel("x position")
    axis.set_ylabel("y position")
    axis.set_title("All agents: true state and estimator posterior")
    axis.set_aspect("equal", adjustable="datalim")
    axis.grid(True, alpha=0.32)
    save_figure(figure, output, dpi)
...
def create_animation(
    ...
    threshold = float(trace["position_trigger_radius"])
    ...
    pre_error = position_error(prior, truth[1:])
    post_error = position_error(posterior[1:], truth[1:])
    time = np.arange(1, horizon + 1)

    x_min, x_max, y_min, y_max = scene_limits(truth, posterior)
    radius_max = max(float(np.max(pre_radii)), float(np.max(post_radii)), threshold, 1e-6)
    error_max = max(float(np.max(pre_error)), float(np.max(post_error)), 1e-6)
    ...
    scene_axis.set_title("True state and covariance-triggered estimator belief")
    ...
    uncertainty_axis.set_title("Remote position uncertainty and threshold")
    ...
    error_axis.set_title("Remote position-estimation error")
    ...
    uncertainty_axis.axhline(
        ...
        linestyle="--",
        linewidth=1.8,
        label=f"trigger radius = {threshold:g}",
    )
    ...
    def update(frame_number: int):
        ...
        triggered = [
            i for i in np.flatnonzero(communications[transition_index])
            if int(i) != ego_agent_id
        ]
        comm_text = "none" if not triggered else ", ".join(f"agent {int(i)}" for i in triggered)
        status.set_text(f"Step {step}\nCommunication: {comm_text}")
    ...
    if output_path.suffix.lower() == ".gif":
        ...
        writer = animation.FFMpegWriter(
            fps=fps,
            bitrate=3500,
            metadata={"title": "Structured covariance-triggered estimator"},
        )
...
def main() -> None:
    ...
    ellipse_scale = (
        float(trace["position_trigger_scale"])
    ...
    plot_all_agents_three_way(
        ...
        communications,
        ego_agent_id,
        args.output_directory / "all_agents_three_way.png",
        args.dpi,
    )
    plot_all_agents_true_estimator(
        ...
        communications,
        ego_agent_id,
        args.output_directory / "all_agents_true_estimator.png",
        args.dpi,
    )
    ...
    print(" - all_agents_three_way.png")
    print(" - all_agents_true_estimator.png")
    print("Animation:", animation_output)
    print(
        "Ego note: agent 0 is corrected by its own observation at every "
        "step, so its true and estimator bodies should nearly overlap."
    )

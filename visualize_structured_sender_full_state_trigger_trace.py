...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=Path(
            ...
            "sender_full_state_trigger_estimator_trace.npz"
        ),
    )
    parser.add_argument(
        "--output-directory",
        type=Path,
        default=Path(
            "results/animations/structured_sender_full_state_trigger"
        ),
    )
    parser.add_argument(
        "--animation-name",
        default="structured_sender_full_state_trigger.mp4",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1.0,
        help="Negative means use position_ellipse_scale saved in the trace.",
    )
...
def load_trace(path: Path) -> dict[str, Any]:
    ...
    required = {
        ...
        "pre_message_full_state_uncertainty_scores",
        "post_message_full_state_uncertainty_scores",
        "pre_message_full_state_error_scores",
        "post_message_full_state_error_scores",
        "communications",
        "sender_full_state_error_threshold",
        "sender_full_state_uncertainty_threshold",
        "sender_uncertainty_sigma_scale",
        "position_ellipse_scale",
        "state_scales",
    }
...
def create_animation(
    ...
    posterior = trace["estimator_posterior"]
    covariances = trace["posterior_covariances"]
    pre_uncertainty = trace[
        "pre_message_full_state_uncertainty_scores"
    ]
    post_uncertainty = trace[
        "post_message_full_state_uncertainty_scores"
    ]
    pre_error = trace[
        "pre_message_full_state_error_scores"
    ...
    error_threshold = float(
        trace["sender_full_state_error_threshold"]
    )
    uncertainty_threshold = float(
        trace["sender_full_state_uncertainty_threshold"]
    )
    ...
    uncertainty_max = max(
        float(np.max(pre_uncertainty)),
        float(np.max(post_uncertainty)),
    ...
    uncertainty_axis.set_ylim(0.0, uncertainty_max * 1.10)
    uncertainty_axis.set_xlabel("environment step")
    uncertainty_axis.set_ylabel("normalized full-state uncertainty")
    uncertainty_axis.set_title("All-agent full-state uncertainty score")
    ...
    error_axis.set_ylabel("normalized full-state RMS error")
    error_axis.set_title("All-agent full-state prediction error")
    ...
    uncertainty_axis.axhline(
        ...
        linestyle="--",
        linewidth=1.8,
        label=f"uncertainty threshold = {uncertainty_threshold:g}",
    )
    ...
    for agent_id in range(num_agents):
        ...
        if np.any(event_mask):
            uncertainty_axis.scatter(
                time[event_mask],
                pre_uncertainty[event_mask, agent_id],
                s=34,
                marker="D",
                facecolor="none",
                edgecolor=COMMUNICATION_COLOR,
    ...
    if output_path.suffix.lower() == ".gif":
        ...
        writer = animation.FFMpegWriter(
            fps=fps,
            bitrate=3500,
            metadata={"title": "Structured normalized full-state sender trigger"},
        )
...
def main() -> None:
    ...
    ellipse_scale = (
        float(trace["position_ellipse_scale"])
    ...
    plot_all_agents_three_way(
        ...
        posterior,
        communications,
        args.output_directory / "all_agents_full_state_sender_three_way.png",
        args.dpi,
    )
    plot_all_agents_true_estimator(
        ...
        / "all_agents_full_state_sender_true_estimator.png",
        args.dpi,
    )
    ...
    print(
        "Sender semantics: every agent evaluates normalized "
        "full-state error and normalized full-state uncertainty. "
        "A private observation enters the common belief only after "
        "broadcast."
    )
